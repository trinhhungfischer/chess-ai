from gui.board import ChessBoard, BoardControls
from gui.player import Player, AiPlayer, PlayerOptions
...
class Window(QWidget):
  ...
  def __init__(self, exe_path, turn_limit_s, thread):
    ...
    if os.path.isfile(exe_path):
      self.player_b = AiPlayer(
        exe_path,
        turn_limit_s,
        Player.BLACK,
        thread,
        self.board)
      self.player_w = AiPlayer(
        exe_path,
        turn_limit_s,
        Player.WHITE,
        thread,
...
if __name__ == '__main__':
  ...
  if not os.path.isfile(args.ai_exe_path):
    ...
    warning.setText(
      "AI exe path '{0}' not found. Associated controls will be disabled.\nRun main.py --help for details.".format(args.ai_exe_path))
    warning.exec()
